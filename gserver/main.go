...
/*
	总结：
	proto 定义请求和响应结构体(消息类型)，定义暴露的方法。方法的参数是请求结构体，返回响应结构体；(我们可以一次性的在一个 .proto 文件中定义服务并使用任何支持它的语言去实现客户端和服务器)
	在 server 中调用 RegisterHelloServer，在 client 调用 NewHelloClient

	server：
	定义一个对象，实现 proto 文件中暴露的方法，请求参数增加 ctx，返回参数增加 err；
	在 main 中实例化这个类，并将这个类注册进实例话的 grpc server
	提供服务即可

	client：
	通过 grpc 和 生成的 proto 文件实例话 client
	构造并实例化请求结构体（基于 pb），通过实例化的 client 发出请求即可！
